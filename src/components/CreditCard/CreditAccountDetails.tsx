    ...
    <>
        ...
        <div className='flex max-w-full text-[18px] md:text-[20px] w-full flex-col gap-2 lg:text-start lg:flex-row p-2'>
          <div className='max-w-full w-full mt-2 md:ml-2'>
            ...
            <p className='mb-1'>
              Type:{" "}
              {cardType === "Gold" ? (
                <span className='font-bold text-green-700'>{cardType}</span>
              ) : (
                <span className='font-bold text-red-700'>Platinum</span>
              )}
            </p>
            <p className='mb-1'>
              Total Balance:
            ...
            <p className='mb-1'>
              Used Balance:{" "}
              <span className='font-bold text-red-700'>Rs. {usedBalance}</span>
            </p>
            <p className='mb-1'>
              Annual Charges:{" "}
          ...
          <div className='max-w-full lg:max-w-3/4 w-full lg:w-3/4 mt-2 text-l flex lg:block'>
            <div className='max-w-full w-full mt-2 mx-0 mr-2 lg:mr-0 lg:mb-2 '>
              <button
                ...
                className='w-full font-alata py-2 px-4 bg-red-600 hover:bg-red-700 active:bg-red-800 rounded-md text-white text-md font-bold disabled:opacity-50'
                onClick={() => {
                  setModal(true);
                  setAddBal("remove");
                }}
            ...
            <div className='max-w-full w-full mt-2 mx-0 lg:mt-4'>
              <button
                className='w-full font-alata py-2 px-4 bg-green-600 hover:bg-green-700 active:bg-green-800 rounded-md text-white text-md font-bold disabled:opacity-50'
      ...
      {modal && (
        <InputModal
          ...
          getAmount={getAmount}
          label={`Enter amount ${addBal === "add" ? "to pay the bill" : ""}`}
        />
      )}
      {load && <LoadingSpinner />}
